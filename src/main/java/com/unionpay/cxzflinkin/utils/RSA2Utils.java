...
public class RSA2Utils {
    ...
    public static byte[] encryptByPrivateKey(byte[] data, byte[] key) throws Exception {
        if (data.length > 245) {
            throw new IllegalBlockSizeException("Rsa encryptByPrivateKey Exception:Data must not be longer than 245 bytes.Data length:" + data.length);
        } else {
            byte[] keyBytes = Base64.decodeBase64(key);
            PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(keyBytes);
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
    ...
    public static byte[] encryptByPublicKey(byte[] data, byte[] key) throws Exception {
        if (data.length > 245) {
            throw new IllegalBlockSizeException("Data must not be longer than 245 bytes.Data length:" + data.length);
        } else {
            byte[] keyBytes = Base64.decodeBase64(key);
            X509EncodedKeySpec keySpec = new X509EncodedKeySpec(keyBytes);
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
